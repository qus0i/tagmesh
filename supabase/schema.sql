...
CREATE POLICY "Admin manage variants" ON product_variants FOR ALL USING (auth.role() = 'authenticated');
...
-- Discount Codes: public read active codes (for validation), admin full access
ALTER TABLE discount_codes ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Public read active codes" ON discount_codes FOR SELECT USING (is_active = true);
CREATE POLICY "Admin manage discount codes" ON discount_codes FOR ALL USING (auth.role() = 'authenticated');
